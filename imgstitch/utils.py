...
def get_matches(self, img_a_gray, img_b_gray, num_keypoints=1000, threshold=0.8):

    Label(self.stitching_log_top_frame, text="Detecting keypoints....", font=("Arial", 25) ).pack(fill="x")
    ...
    draw_keypoints_img_1 = cv2.drawKeypoints(img_a_gray, kp_a, None)
    draw_keypoints_img_2 = cv2.drawKeypoints(img_b_gray, kp_b, None)


    view_keypoints_img1=Label(self.stitching_log_top_frame, text="[View keypoints]",  font=("Arial", 25), fg="green")
    view_keypoints_img1.pack(fill="x")
    view_keypoints_img1.bind("<Button-1>", lambda event: self.displayImage(draw_keypoints_img_1))
    view_keypoints_img2=Label(self.stitching_log_top_frame, text="[View keypoints]", font=("Arial", 25), fg="green")
    view_keypoints_img2.pack(fill="x")
    view_keypoints_img2.bind("<Button-1>", lambda event: self.displayImage(draw_keypoints_img_2))
    self.stitching_step="step 1"
    ...
    draw_matching_img = cv2.drawMatches(
        img_a_gray, kp_a, img_b_gray, kp_b, good_matches_list, None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS
    )

    view_matching_keypoints=Label(self.stitching_log_top_frame, text="[View matching keypoints]",  font=("Arial", 25), fg="green")
    view_matching_keypoints.pack(fill="x")
    view_matching_keypoints.bind("<Button-1>", lambda event: self.displayImage(draw_matching_img))
    self.stitching_step="step 2"
...
def compute_homography_ransac(matches_a, matches_b, self):
...
def stitch_image_pair(self, img_a, img_b, stitch_direc):
    ...
    """
    Label(self.stitching_log_top_frame, text="Turn images into grayscale", font=("Arial", 25)).pack(fill="x")
    
    img_a_gray = cv2.cvtColor(img_a, cv2.COLOR_BGR2GRAY)
    img_b_gray = cv2.cvtColor(img_b, cv2.COLOR_BGR2GRAY)
    matches_a, matches_b = get_matches(self, img_a_gray, img_b_gray, num_keypoints=1000, threshold=0.8)
    Label(self.stitching_log_top_frame, text="Computing homography with RANSAC....", font=("Arial", 25)).pack(fill="x")
    h_mat = compute_homography_ransac(matches_a, matches_b, self)
    self.stitching_step="step 3"
    if stitch_direc == 0:
        ...
        Label(self.stitching_log_top_frame, text="Blending images....", font=("Arial", 25)).pack(fill="x")
        canvas[:, 0:img_a.shape[1], :] = img_a[:, :, :]
        self.stitching_step="step 4"
        Label(self.stitching_log_top_frame, text="Cropping images....", font=("Arial", 25)).pack(fill="x")
